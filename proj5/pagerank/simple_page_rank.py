...
class SimplePageRank(object):
    ...
    def update_weights(nodes, num_nodes):
        ...
        def distribute_weights((node, (weight, targets))):
            # YOUR CODE HERE
            # return (node, (weight given to node, targets)) 
            # return (target, (weight given to target, []))
            keyvals = []
            if (len(targets) > 0):
                ...
                # for other_node in nodes:
                    ...
                    counter -= 1
            keyvals.append((node, (0.05 * weight, targets)))
            return keyvals     
            # each keyval pair is (node, (weight of that node, targets of that node, node_num of a target node, weight given from node to target node))
        ...
        def collect_weights((node, values)):
            # YOUR CODE HERE
            # output should be (node, (weight, targets))
            # put each node in a dictionary (node: weight)
            # iterate through (node, values) pairs
            # if target in values is a dict key, dict key += target weight
            # else add target to the dictionary, dict key = target weight
            # list.append(node(weight from dictionary, targets from values))
            totalweight = 0.1
            alltargets = frozenset()
            for weight, targets in values:
                totalweight += weight
                alltargets = alltargets | targets

            return (node, (totalweight, alltargets))
        ...
        return nodes\
                ...
                # (node, (weight given to key, targets)
                # (target, (weight given to target, []))
                # RDD[(K, V)] (the keys can repeat)
                # RDD[(K, List[V])] (the keys are unique)
