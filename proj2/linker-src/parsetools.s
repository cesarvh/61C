...
hex_to_str:
	...
	li $t0 8   # counter = 0
	li $t4 0 
	addi $t4 $a1 0   # so we dont mutate it
	li $t1 0xf # this will be our bitmask!
	li $t2 0 #will hold our current ASCII char
	sll $t1 $t1 28  # this will allow us to get the most significant bits :)
	li $t6 28 #amount to shift by eah time
	j HexifyLoop

HexifyLoop:
	#beq $t1 $0 endHex # is our bitmask 0? then we're done
	beq $t0 $0 endHex
	and $t2 $t1 $a0 # get the digit! 
	srlv $t2 $t2 $t6 # now shift it X amounts back to get the true number
	slti $t3 $t2 10 # if t2 is < 10, t3 == 1
	bne $t3 $0 isNumeric # if t3 == 1, its a number
	addi $t2 $t2 48 # add 48 
	addi $t2 $t2 39 # this will give me the ASCII val of the number
	sb $t2 0($a1) #store the current digit
	addi $a1 $a1 1 #move the pointer forward
	srl $t1 $t1 4 # shift right 
	addi $t6 $t6 -4 # decrease the amount of shifts made
	addi $t0 $t0 -1 #drcrease counter
	j HexifyLoop

isNumeric:
	addi $t2 $t2 48 # this will give me the ASCII val of the number
	sb $t2 0($a1) #store the current digit
	srl $t1 $t1 4 # shift right 
	addi $a1 $a1 1 #move the pointer forward
	addi $t6 $t6 -4 # decrease the amount of shifts made
...
endHex:
	addi $t5 $t5 10 # insert val of \n to t5
	sb $t5 0($a1) # insert the new line
	addi $a1 $a1 1 # increase ptr
	sb $zero 0($a1) # null char
# restore the stack
	lw $ra 0($sp)
	lw $a0 4($sp)
	addiu $sp $sp 8
