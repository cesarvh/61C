...
write_machine_code_find_text:	
	move $a0, $s1	# a0 has input file ptr
	jal readline
	blt $v0, $0, write_machine_code_error	# Invalid line reached -> branch if less
	move $a0, $v1	#$a0: pointer to buffer with chars read
	la $a1, textLabel # textLabel is .text
	jal streq # $a0 is string 1, $a1 is string 2
	bne $v0, $0, write_machine_code_find_text # if $v0 is -1, go back to loop, else
	...
	addu $s5 $0 $0 # set to 0 and increment by 4 for each instruction read

write_machine_code_next_inst:
	# 2. Call readline() while passing in the correct arguments:
	# YOUR_INSTRUCTIONS_HERE
	#  readline arguments: $a0 = File handle
	move $a0 $s1
	jal readline
	# v0: number of bytes read, v1: pointer to buffer with chars read
	...
	lbu $t0, 0($v1)
	beq $t0, $0, write_machine_code_done # if no more characters, jump, else:
