...
#------------------------------------------------------------------------------
# we need to check if its a jump or a branch
# if it is, then we know what to do
inst_needs_relocation:
    li $v0 0 #prepare the return value
    add $t0 $a0 $0 # create a pointer to a0
    li $t1 2 # set it to j
    li $t2 3 # set it to jal

    srl $t0 $t0 26 # shift it 26 place to the left to get actual value

    beq $t0 $t1 Rel # if its j then yes
    beq $t0 $t2 Rel # if its jal then yes
    addi $v0 $v0 0 # v0 = 0 means no relocation
    j endRel #jump to ened
Rel:
    addi $v0 $v0 1 # v0 = 1 if yes relocation
    j endRel # jump to end
endRel:
    jr $ra #end
...
relocate_inst:	
		...
		addu $a0 $0 $a3 #put the instruction into a0 so we can call s_f_a
		jal symbol_for_addr #call #s_f_a
		beq $v0 $0 does_not_exist #v0 == 0? Means an error happened. Exit.
		
        addu $a0 $0 $a2 # now we load the new parameters
        addu $a1 $0 $v0  # in order to cal a_f_s
		jal addr_for_symbol
		li $t0 -1 
        beq $v0 $t0 does_not_exist #v0 == -1? Error
			
        lw $a0 0($sp) #restore a0 to its original value 
		andi $a0 $a0 0xFC000000 #get the 1st 6 bits
		srl $v0 $v0 2 # get rid of the two least significant bits bc always should be on
		or $v0 $a0 $v0 # now we alter the instruction  which will give us the absolute address or whatever its called
		j end_relocation # jump to the end!
